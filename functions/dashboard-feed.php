...
function rc_mdm_register_widgets() {
	...
	wp_add_dashboard_widget( 'widget_pgplorer_feed', __( 'ProfitGeeks Blog', 'pg' ), 'pg_dashboard_rss_box' );
	//wp_add_dashboard_widget( 'widget_wdexplorer_feed', __( 'WDExplorer Blog', 'pg' ), 'wde_dashboard_rss_box' );
}
add_action('wp_dashboard_setup', 'rc_mdm_register_widgets');

// Creates the RSS metabox for WPExplorer feed
function pg_dashboard_rss_box() {
	
	// Get RSS Feed(s)
	include_once(ABSPATH . WPINC . '/feed.php');
	
	// My feeds list (add your own RSS feeds urls)
	$my_feeds = array( 'http://www.profitgeeks.org/feed/');
	...
	foreach ( $my_feeds as $feed) :
		...
		// End <ul> tag
		echo '</ul><div style="margin-top:20px;padding-top:10px;border-top:1px solid #eee;">Feed from <a href="http://www.profitgeeks.org" target="_blank">profitgeeks.org</a></div></div>';
