...
if (in_array($siw_icons, $siw_custom_sizes)) {
	...
	$siw_abs_path = str_replace('lib/', '', get_template_directory( __FILE__ ));

	/*	Fix for Windows/XAMPP where the slash goes the wrong way.
	 Thanks to VictoriousK */
	$siw_abs_path = str_replace('\\', '/', $siw_abs_path);
		
	$siw_icon_path =  $siw_abs_path . '/functions/social-media/icons/'.$siw_icons.'/'.$id.'.{gif,jpg,jpeg,png}';

	if($siw_icons == 'large') {
		$imgsize = 'height="64" width="64"';
	}
	elseif($siw_icons == 'medium') {
		$imgsize = 'height="32" width="32"';
	}
	elseif($siw_icons == 'small') {
...
if($result) {
	if (in_array($siw_icons, $siw_custom_sizes)) {
		...
		$siw_icon = home_url().'/wp-content/themes/'.$siw_path[1];
	}
	else {
		$siw_path = explode('functions', $result[0]);
		$siw_icon = get_template_directory_uri().'/functions'.$siw_path[1];
	}
}
elseif( $siw_labels != 'show' && $siw_icons != 'small' ) {
	$siw_icon = get_template_directory_uri().'/functions/social-media/icons/'.$siw_icons.'/_unknown.jpg';
}
else {
	$siw_icon = '';
}
...
if ( $siw_icon ) {
	$siw_data['image'] = '<img class="site-icon" src="'.$siw_icon.'" alt="'.$siw_title.'" title="'.$siw_title.'" '.$imgsize.' />';
}
else { $siw_data['image'] = '';
}
